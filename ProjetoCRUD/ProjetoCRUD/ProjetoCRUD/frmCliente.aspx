<%@ Page Title="" Language="C#" MasterPageFile="~/frmPrincipal.Master" AutoEventWireup="true" CodeBehind="frmCliente.aspx.cs" Inherits="ProjetoCRUD.frmCliente" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="AreaDeTrabalho" runat="server">
    <!DOCTYPE html>
        <html>
            <body>
                <!-- Criando o formulário -->
                <fieldset>
			            <div class="Dados">	
                            <br />
				            <h3 id="inserirDados">Insira os Dados abaixo:</h3><span>&nbsp ID: 
                            <asp:TextBox ID="txbId" runat="server" Width="44px"></asp:TextBox><br />
                            </span><br />
                            <div class="nome">Nome:<br>
					            <asp:TextBox type="text" name="txbNome" runat="server" required="required" autofocus="" ID="txbNome" ></asp:TextBox>
				            </div>
				            <div class="nasc">Sobrenome:<br>
					            <asp:TextBox type="text" name="txbSobrenome"  runat="server" required="required" ID="txbSobrenome"></asp:TextBox>
				            </div><br />
                            <br>
				            <div class="email">Email:<br>
					            <asp:TextBox type="text" name="txbEmail" runat="server" required="required" ID="txbEmail"></asp:TextBox>
				            </div>
				            <div class="cpf">CPF:<br>
					            <asp:TextBox type="text" name="txbCpf" runat="server" required="required" ID="txbCPF"></asp:TextBox>
				            </div><br />
				            <br />
				            <div class="telefone">Telefone:<br>
					            <asp:TextBox type="text" name="txbTelefone" runat="server" required="required" ID="txbTelefone"></asp:TextBox>
				            </div>
				            <div class="cep">Cep:<br>
					            <asp:TextBox type="text" name="txbCep" runat="server" required="required" ID="txbCEP"></asp:TextBox>
				            </div><br />
				            <br>
				            <div class="rua">Rua:<br>
					            <asp:TextBox type="text" name="txbRua" runat="server" required="required" ID="txbRua"></asp:TextBox>
				            </div>
				            <div class="ncasa">Nº:<br>
					            <asp:TextBox type="text" name="txbNCasa" runat="server" required="required" ID="txbNumero"></asp:TextBox>
				            </div><br><br />
				            <div class="bairro">Bairro:<br>
					            <asp:TextBox type="text" name="txbBairro" runat="server" required="required" ID="txbBairro"></asp:TextBox>
				            </div>
				            <div class="cidade">Cidade:<br>
					            <asp:TextBox type="text" name="txbCidade" runat="server" required="required" ID="txbCidade"></asp:TextBox>
				            </div>
				            <!-- "placeholder" é a palavra que fica no input antes de preenche-lo" -->
			            </div>
                </fieldset>
			            <br>
                <div class="botoes">
                        <asp:Button class="btn btn-success" ID="btnSalvar" runat="server" Text="Salvar" OnClick="btnSalvar_Click" />
                        <asp:Button class="btn btn-secondary" ID="btnLimpar" runat="server" Text="Limpar" OnClick="btnLimpar_Click" />
                        <asp:Button class="btn btn-danger" ID="btnExcluir" runat="server" Text="Excluir" OnClick="btnExcluir_Click" />
                        <br>
                </div>    
                    <asp:GridView ID="gvConsultaClientes" runat="server" AutoGenerateColumns="False" CellPadding="20" ForeColor="#333333" OnSelectedIndexChanged="gvConsultaClientes_SelectedIndexChanged" Width="300px">
                        <AlternatingRowStyle BackColor="White" />
                        <Columns>
                            <asp:CommandField ShowSelectButton="True" />
                            <asp:BoundField DataField="CodCliente" HeaderText="Código" Visible="true">
                            <ControlStyle/>
                            <ItemStyle />
                            </asp:BoundField>
                            <asp:BoundField DataField="Cl_nome" HeaderText="Nome" Visible="true">
                            <HeaderStyle HorizontalAlign="Center"/>
                            <ItemStyle HorizontalAlign="Center"/>
                            </asp:BoundField>
                            <asp:BoundField DataField="Cl_sobrenome" HeaderText="Sobrenome" Visible="true" >
                            <ItemStyle HorizontalAlign="Center" />
                            </asp:BoundField>
                            <asp:BoundField DataField="Cl_cpf" HeaderText="CPF" Visible="true">
                            <ItemStyle HorizontalAlign="Center" />
                            </asp:BoundField>
                            <asp:BoundField DataField="Cl_telefone" HeaderText="Telefone" Visible="true">
                            <ItemStyle HorizontalAlign="Center" />
                            </asp:BoundField>
                            <asp:BoundField DataField="Cl_email" HeaderText="Email" Visible="true">
                            <ItemStyle HorizontalAlign="Center" />
                            </asp:BoundField>
                            <asp:BoundField DataField="Cl_rua" HeaderText="Endereco" Visible="true" >
                            <ItemStyle HorizontalAlign="Center" />
                            </asp:BoundField>
                            <asp:BoundField DataField="Cl_numero" HeaderText="Numero" Visible="true">
                            <ItemStyle HorizontalAlign="Center" />
                            </asp:BoundField>
                            <asp:BoundField DataField="Cl_bairro" HeaderText="Bairro" Visible="true">
                            <ItemStyle HorizontalAlign="Center" />
                            </asp:BoundField>
                            <asp:BoundField DataField="Cl_cidade" HeaderText="Cidade" Visible="true" >
                            <ItemStyle HorizontalAlign="Center" />
                            </asp:BoundField>
                            <asp:BoundField DataField="Cl_cep" HeaderText="Cep" Visible="true" >
                            <ItemStyle HorizontalAlign="Center" />
                            </asp:BoundField>
                
                            <asp:BoundField />
                
                        </Columns>
                        <EditRowStyle BackColor="#7C6F57" BorderWidth="15px" HorizontalAlign="Right" Width="800px" />
                        <FooterStyle BackColor="#1C5E55" Font-Bold="True" ForeColor="White" />
                        <HeaderStyle BackColor="#1C5E55" Font-Bold="True" ForeColor="White" Width="500px" />
                        <PagerStyle BackColor="#666666" ForeColor="White" HorizontalAlign="Center" />
                        <RowStyle BackColor="#E3EAEB" Wrap="True" />
                        <SelectedRowStyle BackColor="#C5BBAF" Font-Bold="True" ForeColor="#333333" />
                        <SortedAscendingCellStyle BackColor="#F8FAFA" />
                        <SortedAscendingHeaderStyle BackColor="#246B61" />
                        <SortedDescendingCellStyle BackColor="#D4DFE1" />
                        <SortedDescendingHeaderStyle BackColor="#15524A" />
                    </asp:GridView>
            </body>
        </html>
    </asp:Content>
